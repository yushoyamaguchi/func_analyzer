...
fn parse_c_fn(source: &Vec<&str>, fn_node: &Arc<Mutex<FunctionNode>>) {
...
fn search_c_fn(source: &Vec<&str>, depth:i64, fn_node: &Arc<Mutex<FunctionNode>>) {
...
pub fn generate_call_graph(source: &Vec<&str>, depth: i64, root: Arc<Mutex<FunctionNode>>)  {
    let src_c_file = source;
    let depth = depth;
    search_c_fn(source, depth, &root);
